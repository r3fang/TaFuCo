...
//		if(s_kmer->count == 1){
//			gene = get_gene_name(strdup(s_kmer->seq_names[0]));
...
int main(int argc, char *argv[]) {
	...
	if (argc != 6) {  
	        fprintf(stderr, "Usage: %s <in.fa> <read_R1.fq> <read_R2.fq> <int k> <int min_weight>\n", argv[0]);  
	        return 1;  
	 }
	...
	int k, min_weight;
	if (sscanf (argv[4], "%d", &k)!=1)	die("Input error: wrong type for k\n");
